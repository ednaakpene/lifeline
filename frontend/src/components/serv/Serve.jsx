      ...
      <div className="flex justify-center mt-10">
        <div className="bg-white shadow-md rounded-lg w-full">
          <div className="flex border-b">
            <button
              className={`w-full py-2 px-4 text-center focus:outline-none ${activeTab === 'Hospitals' ? 'bg-teal-500' : 'hover:bg-teal-200'}`}
              onClick={() => handleTabClick('Hospitals')}
            >
              Hospitals
            </button>
            <button
              className={`w-full py-2 px-4 text-center focus:outline-none ${activeTab === 'Pharmacies' ? 'bg-teal-500' : 'hover:bg-teal-200'}`}
              onClick={() => handleTabClick('Pharmacies')}
            >
              Pharmacies
            </button>
            <button
              className={`w-full py-2 px-4 text-center focus:outline-none ${activeTab === 'Laboratories' ? 'bg-teal-500' : 'hover:bg-teal-200'}`}
              onClick={() => handleTabClick('Laboratories')}
            >
              Laboratories
            </button>
            <button
              className={`w-full py-2 px-4 text-center focus:outline-none ${activeTab === 'Appointments' ? 'bg-teal-500' : 'hover:bg-teal-200'}`}
